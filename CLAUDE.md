...
| 案 | 内容 | 工数 |
|---|---|---|
| C（推奨） | NMFテンプレートの帯域設計を見直す＝判別部分だけ改善 → adt_checkで再判定 | 半日〜1日 |
| A | A2＝専用ADTモデルを隔離venvで導入（Cが外れたら） | 数日〜2週間 |
| B | 自動下書きを使わず、編集UIと簡略化ボタンの価値だけドラマーに検証してもらう | 即 |
- **ドラマーテストは延期**。今見せると採譜の問題でアプリ全体が否定され、何も学べない。
