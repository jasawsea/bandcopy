...
**未了・次の一手（2026-08-02 更新）**
- ~~GitHub公開~~ → **完了**（2026-07-27・上記セクション参照。https://github.com/jasawsea/bandcopy ）。
- ~~ADT A1を実装して採用~~ → **実装済みだが実曲では使えないと判明（2026-08-02）**。
  **最優先タスク＝ADTの楽器判別の修正**。詳細は下の
  「【重要・要対応】ADT A1は実曲では譜面になっていなかった」参照。
  - まず案C（NMFテンプレートの帯域設計見直し・半日〜1日）→ `adt_check.py` で再判定
  - 駄目なら案A（A2＝専用ADTモデルを隔離venv・数日〜2週間）
- **ドラマーへのユーザーテストは延期中**（ADTが直ってから。今見せると採譜の問題で
  アプリ全体が否定され学びが得られない）。テスト手順は設計済み。

**既知の限界（引き継ぎ注意）**
- ~~**ADTの出力は現状そのままでは譜面にならない**~~ → **解決（2026-08-05）**。adt_check合格。
  原因はグリッドの位相（t=0起点）と、活性の大きさに対する独立ピーク検出だった。下記参照。
- ~~フル尺・多数の実曲での検証はまだ~~ → フル尺4分は検証済み（性能は問題なし・2026-08-02）。
  実曲での検証は**3曲**（Rebound／MOREカバー／MOON AIカバー・2026-08-05にADT合格を確認）。
- **配布形態の制約＝依存が2.7GBある**（2026-08-05 計測）。内訳の上位は
  TensorFlow 1.0GB／PyTorch 529MB／llvmlite 125MB／music21 113MB。
  → **単体アプリ化（PyInstaller等）は非現実的**。配布物2〜3GB＋初回のモデルDL、
    Mac/Windows別ビルド、未署名だとGatekeeperで開けない（正規化はApple Developer 年$99）、
    計算はメンバーのPC任せ、修正のたび再配布。**やるならホスティング（HF Spaces）**。
  → 対してメンバーが実際に要るものは **譜面4.5MB**（練習音源込みでも184MB）。
    「アプリを配る」より「出力を配る」ほうが速いことを数字で確認済み。
- **TensorFlow 1.0GB は実際には使っていないが、簡単には外せない**（2026-08-05 確認）。
  `requirements.txt` に既記のとおり basic-pitch は TF/CoreML/TFLite/ONNX の4形式を同梱し、
  本プロジェクトは **ONNX版で動かしている**（TF 2.16以降はKeras 3化でTF版モデルを読めないため）。
  自前コードに `import tensorflow` は**1箇所も無い**（grep確認済み）。
  ただし `basic-pitch` が `Requires: tensorflow-macos` と宣言しているので `pip` は必ず入れる。
  → 外すには `--no-deps` 系の細工が要り、**未検証**。
  → **常設ホスティング（HF Spaces）に進むなら最初に潰す価値がある**（2.7GB→1.7GB／起動も軽くなる）。
    逆にローカル運用を続けるなら触る必要はない。
- タブの運指は heuristic（最適保証なし）。4分離の混在ギターは高フレットが散見。
- 音程パートの採譜精度7〜8割前提（AIが8割・人が2割直す設計）。
- **合成音での合格を実力の根拠にしないこと**（実曲と条件が違いすぎる。上記の教訓参照）。
...
- **実機評価（Rebound・チューニング後）**：SN 4.5→2.4/小節（過検出解消）、HH 空→8分復活、
  ...
  > ⚠️ **この評価は2026-08-02に無効と判明**。ここで見ているのは「1小節あたり何発か」という
  > **密度だけ**で、打点の位置が正しいかを検証していない。実際には拍と噛み合っておらず
  > 譜面として使えなかった。詳細は下の「【重要・要対応】ADT A1は実曲では譜面になっていなかった」参照。
...
- ~~ドラムレーン定義が4箇所に散在~~ → **解決済み（2026-08-01）**。`app/lanes.py`が単一ソース。
  **クラッシュ/ライドを足すときは`app/lanes.py`に1行足すだけでよい**（JS・HTMLは触らない）。
- ~~未反映のドキュメント~~ → **更新済み（2026-08-01）**。`README.md`は`serve_all.py`を推奨導線に、
  `手順書.md`は「第0部 ブラウザで使う」を新設（メンバーはそこだけ読めば足りる）。
- ~~Colab公開トンネルが`/editor`まで通るか~~ → **解決済み（2026-08-01・下記セクション参照）**。

### 【重要・要対応】ADT A1は実曲では譜面になっていなかった（2026-08-02）
実曲で検証した結果、**自動ドラム採譜(A1)の出力は曲の拍と噛み合っていない**ことが判明。
2026-07-29 の「A1採用」判断は**密度しか見ておらず、打点の位置を検証していなかった**。
判断根拠が崩れたのでここに訂正を記録する。

**検証した素材**
- EARTHSHAKER「MORE」のカバー（YouTube・演奏者本人の投稿／4分07秒・152BPM・157小節）
  ※やっさん指定。高難度ジャパメタ＝ダブルキックが期待できる本命の試験材料
- Rebound（`output/Yvv4RVQzIFk`）＝**A1をチューニングしたその素材**でも再検査

**動作面は問題なし（ここは合格）**
- 取得〜Demucs分離30秒／自動下書き3.3秒／15,072マスの描画も引っかかりなし。
- 4分の曲でも実用速度。レビューで懸念された「1万マスで重い」は発生しない（再描画8ms）。

**しかし中身が音楽になっていない**
```
MORE     : キック拍頭25% / スネア2拍4拍 12%   （でたらめ＝25% / 12.5%）
Rebound  : キック拍頭25% / スネア2拍4拍 10%   （同上）
```
ロックならスネアは2拍4拍に7〜9割集中するはず。**偶然と同じ＝全16ステップに均等にばらけている。**
一見するとMOREでダブルキック38箇所・12連打1箇所を検出して「本丸が動いた」ように見えるが、
位置がでたらめなので譜面として使えない。

**原因＝楽器の判別が壊れている（オンセット検出は正常）**
段階ごとに切り分けた結果：
| 段階 | 状態 |
|---|---|
| オンセット検出（打点の時刻） | **正常**。約50%がビート上（偶然の2〜3倍） |
| 楽器の判別（キックかスネアか） | **壊れている** |
```
Rebound : キック62個のうち 94% を、スネアも「同じ瞬間」だと主張
MORE    : キック708個のうち 87% を、スネアも同時に主張
MORE    : 素のオンセット1140個に対しスネア成分が1108個に反応（＝ほぼ全打点に反応）
```
NMFの固定テンプレート（`build_drum_templates`）が機能しておらず、
**「音が鳴った」ことは分かるが「どの太鼓か」を区別できていない。**
位相（小節の開始位置）のズレも疑って全オフセットを走査したが、最良でも2拍4拍19%止まり＝
位相は主因ではない。

**合成音テストが通ったのに実曲で失敗した理由（教訓）**
正解既知の合成ドラムでは キック56/56・誤検出0・ダブル/トリプルも完全一致し、
「キック間引き」も56→32と機能した。だが合成音は①スペクトルが理想的に分離しやすく
②時刻0ちょうどから始まる。**実曲の条件を代表していなかった。**
→ 今後ADTを触るときは合成音の合格を根拠にしないこと。

**再発防止：検査スクリプトを追加**
`./venv/bin/python adt_check.py <ドラムwav> [--tempo N]`
- ①打点の位置が音楽的か（スネアの2拍4拍占有率が偶然の2倍を超えるか）
- ②キックとスネアを判別できているか（同一時刻の重複主張が50%未満か）
- 両方通れば終了コード0。**ADTを直したらこれで合否判定する。**

**方針＝案C に決定（2026-08-02・やっさん判断）／着手は 2026-08-05(水) 以降**
判別部分だけを直して `adt_check.py` で再判定する。外れたら案A（A2＝専用ADTモデルを
隔離venv・数日〜2週間）に上げる。**ドラマーテストはそれまで延期。**

> **2026-08-05 実施済み → 合格。案A(A2)には上げない。**
> ただし「判別だけが原因」という前提は誤りで、主犯はグリッドの位相だった。
> 上の分析（原因＝楽器の判別／87〜94%の同時主張）は**次セクションで訂正している**。

### ADT修正＝案C 実施・adt_check合格（2026-08-05）

**結果：実曲3素材すべてで合格。** ドラマーに見せる段階に進んでよい。

| 素材 | スネア2拍4拍 | スネア拍頭 | KK/SN同時主張 |
|---|---|---|---|
| MORE（152BPM・157小節） | 12% → **44%** | 22% → **67%** | 37%（基準50%未満） |
| Rebound 35秒（86BPM） | 12% → **44%** | → **81%** | 3% |
| Rebound 全尺245秒（86BPM） | → **40%** | → **81%** | 3% |

速度は4分の曲で4.3秒（旧3.3秒）。テスト 129 → **144緑**。
gitブランチ `fix-adt-discrimination`。

#### ⚠️ 前回(2026-08-02)の診断は2点まちがっていた。訂正する

**訂正1：挙げていた原因候補（floor除去・帯域の重なり・残差成分・勝者総取り）は全部外れ。**
4条件を実測で総当たりしたが、**どれもスネアの2拍4拍占有率を10〜12%から1ポイントも動かせなかった**。
勝者総取りは重複を0%にするが、位置はでたらめのまま。

**訂正2：「キックの87〜94%をスネアも同時主張」は測定方法の産物だった。**
旧`adt_check.py`の基準②は、採譜本体とは**別のFFT設定**(2048/512。本体は1024/256)でNMFを回し直し、
**ユーザーに届かない内部の活性**を測っていた。実際の出力グリッドで測り直すと旧アルゴリズムでも
重複は**30%**（＝基準を満たしていた）。重複は最初から主犯ではなかった。
→ 基準②は出力グリッドを測るよう直した（判定値50%未満は据え置き）。
→ **教訓：検査は必ず「ユーザーが受け取るもの」を測る。内部状態を測ると存在しない病気を診断する。**

#### 本当の原因は2つ（どちらも実測で確認）

**原因1：グリッドが t=0 起点だった（これが主犯）**
`transcribe_drums`は`step_times = s * (4*60/tempo)/16`を**時刻0から**敷いていた。だが実際の1拍目は
MOREで t=0.255s にある。実測で「**拍がグリッドの拍位置(4の倍数step)に乗る割合 17%**」＝偶然25%を
下回っていた。テンポ自体は正しく（ズレは最大0.48ステップ）、**位相だけが壊れていた**。
※前回「全オフセットを走査したが最良19%」とあるが、一律のオフセットでは直らない。拍そのものに
  合わせる必要がある。

**原因2：判別が活性の「大きさ」に対する独立ピーク検出だった**
低域は残響で大きさが常に高いので、打点でない場所でも反応する（2026-07-25のPoCで既に
「低域残響が強くほぼ全部キック判定」と観測されていた症状と同じ）。

#### 決め手になった観測

NMFを迂回して生の帯域フラックス（＝立ち上がり）を拍ごとに平均すると、**グルーヴは極めて明瞭に出ていた**：
```
MORE  低域30-120Hz  拍mod4: 0.34 / 1.26 / 0.37 / 2.02   ← キックが拍1・3に6倍集中
MORE  胴150-400Hz   拍mod4: 1.40 / 0.56 / 1.35 / 0.70   ← スネアが拍0・2に集中（相補的）
```
**音楽情報は生データに最初から存在していた。パイプラインが壊していただけ。**

#### 直した内容（`app/drum_transcribe.py` をほぼ全面入れ替え）

1. **グリッドを実際の拍に合わせる**：`librosa.beat.beat_track`で拍を取り、拍列を線形補間して
   ステップ時刻を作る（`beat_step_times`）。**テンポ揺れにも追従する**。
   小節の位相は「低域の立ち上がりが小節頭に集まる向き」を選ぶ（`choose_bar_phase`）。
2. **ビート追従のオクターブ誤りを補正**（`reconcile_beats`）：beat_trackはテンポの倍・半分を
   掴みやすい。**合成音テストで実際に120BPMを60BPMと誤検出して発覚した**。呼び出し側が渡す
   テンポを正として拍間隔を分割/間引きする（位相と揺れは追従結果を活かす）。
3. **NMFを廃止し、帯域フラックス（半波整流微分）に変更**。「大きさ」でなく「立ち上がり」を見る。
4. **オンセット起点をやめ、グリッド起点に変更**：各ステップの±半ステップ窓でフラックス最大値を取り、
   その帯域自身の95パーセンタイルの0.5倍を超えたら打点（`hits_from_values`）。
   曲の音量に依存せず、**1ステップ1レーンにつき最大1打点なので過検出が構造的に起きない**。
- 削除：`build_drum_templates`／`nmf_activations`／`_onsets_from_activation`／`_band`／
  `quantize_onsets_to_grid`／`remove_ghost`（全て新方式では未使用）。
- しきい値`HIT_THRESHOLD_FRAC=0.5`は**3素材で0.40〜0.70が合格域**。その中央を取った（過学習ではない）。

#### テスト（`tests/test_drum_groove.py` を新設）

**打点の「位置」を見るテストを置いた。** 数や形だけ見るテストでは2026-08-02の失敗を検出できなかった。
頭に半端な無音(0.31秒)を置いた8ビート合成音で、スネアがstep4・12に50%以上集中することを要求する。
**このテストは修正前のコードで実際に落ちることを確認済み**（キックがstep 2,3,10,11＝無音ぶんズレて出た）。
- ただし**合成音の合格は根拠にしない**（2026-08-02の教訓）。本番の合否は実曲の`adt_check.py`。

#### 合否判定（今後もこれで確認する）
```
./venv/bin/python adt_check.py output/_editor/htdemucs/rahKDkNFkrs/drums.wav --tempo 152
./venv/bin/python adt_check.py output/_editor/htdemucs/Yvv4RVQzIFk/drums.wav --tempo 86.13
./venv/bin/python adt_check.py output/_editor/htdemucs/fulltest/drums.wav --tempo 86.13
```
- 目標：スネアの2拍4拍占有率 **25%以上** ／ キックとスネアの同時主張 **50%未満**
- 両方通れば終了コード0。**通るまでドラマーには見せない。**

**検証素材（取得済み・すぐ使える）**
- `output/_editor/htdemucs/rahKDkNFkrs/drums.wav`（MORE/EARTHSHAKERカバー・152BPM・157小節）
- `output/_editor/htdemucs/Yvv4RVQzIFk/drums.wav`（Rebound 35秒抜粋・86BPM）
- `output/_editor/htdemucs/fulltest/drums.wav`（Rebound 全尺245秒・86BPM）
- 元音源は `audio/` にある。Demucs再実行は不要。

### ドラマーテスト実施・その結果に沿った修正（2026-08-09）

**ドラマーの回答＝「フィルは欲しい」「倍音が多くてきちんと取れていない」。**
これに沿って直した。テスト 189 → **210緑**。実曲は**6曲**に増えた
（MORE／RADIO_MAGIC／MOON／GLAMOROUS_SKY／Rebound／The_Best_Song）。
出力フォルダは動画IDから**曲名に改名済み**。

#### 直したこと

**① ハイハットが偽物だった** — 検出しておらず、密度から決めた8分を全小節に敷いていた。
MOREの157小節すべてが同じ型だった（「譜面がWAVと違う」の正体）。実検出に変更し、
小節ごとの型が **1種類 → 65〜79種類** になった。`regular_hihat=True` で旧挙動に戻せる。

**② 完コピ寄り＋倍音の混信抑制** — しきい値 0.5 → **0.25**（細部を拾う）。
ただし下げるだけだと重複が38%→67%に悪化するので `suppress_bleed()` と対で使う。
同じステップでKK/SNが両方立ったら、相手より明確に弱い方（<0.8倍）を落とす。
結果 **重複15%・2拍4拍53%**。**HHは抑制対象外**（KK/SNと同時に鳴るのが普通）。
→ 副産物で The_Best_Song が合格に転じた（あの重複88%は倍音そのものだった）。

**③ ベースが倍音を音符にしていた** — 音域は正常（音域外0%）でオクターブ誤りではなく、
**1音に対し基音と倍音を同時刻の別音として出力**していた。難易度3の「和音3音まで」に
基音＋倍音2つが収まるため素通り（`keep=low` は4音以上でしか働かない）。
`PartSpec.max_chord_notes` を追加し**ベースは1音固定**に。
実測 同時刻のオクターブ重なり **183→0 / 71→0 / 97→0**、音符数15〜18%減。

**④ URL取り込みの403** — yt-dlp が「JSランタイムが無い」と警告していた。既定は deno のみで、
Macに入っている node が使われていなかった。PATH上の deno/node/bun を自動検出して渡す。

#### タム（フィル）＝drumsepで試したが**不採用**

`gridshiftstudio/drumsep-onnx`（MIT・ONNXなのでコード実行の懸念なし）を実装。
`app/drumsep.py` として残してある。前後処理はdemucs本体の `_spec/_magnitude/_mask/_ispec`
を借りて繋ぎ、**足し戻し検証**（4本の和が元音源に戻るか）で正しさを機械的に確認した
（誤差 Rebound 1.9% / MORE 4.9%）。

**しかし採譜には使えなかった。** GLAMOROUS_SKYで判明：
- **drumsepがスネアをタム側に振り分ける**。タムRMS(30.0%)がスネア(27.3%)を上回る異常。
  タム検出740個＝1小節4.4回は非現実的
- そのためスネア音源は痩せ、2拍4拍が **55% → 21%**（ヒストグラムが平坦＝検出不能）
- キックだけ分離音源にする案（A案）も実測して **拍頭 38%→41%／31%→32%** と誤差程度。
  1曲5分の分離コストに見合わない

→ **既定では使わない。** `transcribe_drums(kick_wav=...)` の口だけ残した。
→ ただし **`app/drumsep.py` は練習音源としては有用**（キックだけ・スネアだけを聴いて合わせる）。

**※訂正**：途中で「キック拍頭 38%→55%」と報告したが、あれは全レーンを分離音源で採る版の
数字でキック単独の効果ではなかった。上の実測値が正しい。

#### 配布まわり（2026-08-08〜09）

- 画面に**動画URL欄**と**切り出し（開始・終了）欄**を追加。楽譜づくり・ドラム編集の両方
- **ドラムの自動採譜をパイプラインに組み込み**。音源を投げた時点で `drum_grid.json` と
  `midi/ドラム.mid` が出る（以前はエディタを開かないと作られず、バンド譜の最下段が
  テンプレート8ビートで代用されていた）
- バンド譜・タブ譜に**コードネーム**。`chords.json` に保存するので**出力フォルダを改名しても
  元音源を消しても消えない**（以前はフォルダ名の慣習で音源を探しており、改名で消えた）
- **常設URLは作れない**と結論（下の「配布形態の制約」参照）

### ▶ 次回の再開ポイント（ここから始める）

**LarsNet の実装から。フィル（タム）を採るための再挑戦。**

重みは取得・検証済みで `models/larsnet/` にある（git管理外・`取得メモ.md` 参照）。
- **安全確認は済んでいる**：5つとも `weights_only=True` で読める＝純粋な状態辞書で
  コード実行の余地なし。**必ずこの読み方をすること**（demucsは`False`固定なので使わない）
- SHA-256を記録済み。配布元が差し替わったら検知できる
- **ライセンス CC BY-NC 4.0＝非商用限定**。個人利用は可だが仕事に絡めるなら差し替えが要る

残作業：U-Netを組んで推論を通し、**drumsepと同じ評価**をかける。
1. 足し戻し検証（`app/drumsep.py` の `reconstruction_error` が流用できる）
2. スネアの2拍4拍が帯域方式の55%を保てるか（drumsepは21%に落ちて失格）
3. タムの数が現実的か（drumsepは740個＝1小節4.4回で破綻）

**外れたらフィルは諦めてよい。** エディタで手入力すれば数分で済む部分であり、
drumsepで既に一周外している。深追いする性質の課題ではない。

<details><summary>旧・再開ポイント（2026-08-05時点）</summary>

**ADTは合格したので、次は本丸＝ドラマーテスト。**
1. MOREとReboundの自動下書きを譜面に出してドラマーに見せ、「叩けるか」を聞く。
   → ここで初めて「簡略化」の本丸（ダブルキック等をどう間引くか）が実データで議論できる。
2. 残っている精度の弱点（下記）は、ドラマーの反応を見てから優先度を決める。

**今わかっている弱点（急ぎではない）**
- **スネアの再現率が低め**：Reboundは全尺245秒(88小節)で112打点＝1.3打点/小節。ロックなら
  2打点/小節が期待値なので取りこぼしている。しきい値を0.40に下げると増えるが重複も増える。
  「取りこぼし」と「余計な打点」のどちらが人の直しやすいかは**ドラマーに聞いて決めるのが早い**。
- **MOREのキック拍頭が32%**（偶然25%に近い）。ただしMOREはダブルキックで16分に散る曲なので
  musically妥当な可能性が高い。他のロック曲で確かめたい。
- **検証曲がまだ実質2曲**（Rebound／MOREカバー）。`audio/yHMozkPt8Ng.mp3`は未分離のまま。
- ブラウザでの「自動下書き」ボタンの目視確認は**この回では未実施**（preview経由のlocalhost遷移が
  環境で拒否された）。HTTP経路(`POST /auto-draft` 200・形正常)と譜面/MIDI書き出しはコードで確認済み。

</details>

### ドラムレーン定義を単一ソース化（2026-08-01）
`app/parts.py`（音程パート）のドラム版として **`app/lanes.py`** を新設し、4箇所に散在していた
定義を集約した。**レーンを増やすときはこのファイルだけ直せばよい。**
- `LaneSpec` に key/日本語ラベル/記譜位置(step,octave,notehead)/GMノート番号/CSSクラス/ADT対象か を集約。
- 旧`LANE_NOTATION`・`LANE_MIDI_NOTE`（`app/grid.py`）は`lanes.py`から導出する別名として残置（後方互換）。
- **JS側はハードコードをやめ、サーバが埋め込む`window.BANDCOPY_LANES`を読む**
  （`editor.html`で`{{ lanes|tojson }}`。`app/server.py`が`lanes.editor_payload()`を渡す）。
  → レーン追加でJSを触る必要がない。逆戻り防止のテストも置いた。
- **2つの並びは別物**：`_LANES`＝記譜位置の高い順（`grid_to_score`の声部順。変えると楽譜が変わる）／
  `_EDITOR_ORDER`＝エディタ表示順（タムを隣接させるUI都合）。
- **実証**：`lanes.py`にクラッシュ(CR)を1行足すだけで、エディタ7レーン表示・グリッドにCR追加・
  MIDIにGM49出力・譜面描画200 までブラウザで通ることを確認（実験後に復元）。
- テスト 119 → **129**。
