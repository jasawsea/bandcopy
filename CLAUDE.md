...
**既知の限界（引き継ぎ注意）**
- **ADTの出力は現状そのままでは譜面にならない**（打点の位置がでたらめ・上記参照）。
...
**直す対象**：`app/drum_transcribe.py` の `build_drum_templates()` と `_band()`（68〜83行付近）

現状のテンプレートはこう：
```python
kk = _band(freqs, 30, 120)                                     # キック
sn = _band(freqs, 150, 400) + 0.5 * _band(freqs, 2000, 8000)   # スネア（胴＋ノイズ）
hh = _band(freqs, 6000, sr / 2)                                # ハイハット
```
`_band` は帯域内=1.0・**帯域外も floor=0.01** の矩形。ここに原因の候補が3つある：

1. **帯域が重なっている**：スネアの高域(2000-8000)とハイハット(6000-)が重複。
2. **floor=0.01 が全帯域に効く**：キックは打点で広帯域の音が出るため、
   floor経由でスネア成分も一緒に反応してしまう（＝87〜94%の同時主張の正体と推定）。
3. **帯域幅の差**：キックは30-120Hz（ビン数が少ない）／ハイハットは6000Hz〜ナイキスト
   （ビン数が多い）。列正規化しているのでビン単位の重みが大きくずれる。

**打ち手の候補（安い順）**
- **(a) 勝者総取りにする**＝今は成分ごとに独立してオンセットを拾い、全部採用している。
  同じ時刻に複数が反応したら**活性が最大の成分だけに割り当てる**よう変える。
  `transcribe_drums` の中で各レーンを埋める前に一手入れるだけ。**重複87〜94%に直接効く。**
- (b) floor を下げる／0にする、帯域の重なりを解消する
- (c) 残差成分（4本目の「その他」テンプレート）を足して、広帯域エネルギーを
  そこに吸わせる（3本で全部を説明しようとする無理をなくす）
- (d) 帯域幅でビン単位の重みを正規化する
...
```
- 目標：スネアの2拍4拍占有率が **25%以上**（現状10〜12%＝偶然）／
  キックとスネアの同時主張が **50%未満**（現状87〜94%）
- 両方通れば終了コード0。**通るまでドラマーには見せない。**

**検証素材（取得済み・すぐ使える）**
- `output/_editor/htdemucs/rahKDkNFkrs/drums.wav`（MORE/EARTHSHAKERカバー・152BPM・157小節）
- `output/_editor/htdemucs/Yvv4RVQzIFk/drums.wav`（Rebound・86BPM）
